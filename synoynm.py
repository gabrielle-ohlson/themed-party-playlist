...
def topic_scorer(phrase, topic, sim_thresh = 0.6, return_hits = False):
	...
	hits = [phrase[w] for w in range(len(phrase)) if word_scores[w] >= sim_thresh]
	
	above_thresh = list(filter(lambda s: s >= sim_thresh, word_scores))
	if len(above_thresh): avg_score = np.sum(above_thresh)/len(above_thresh)
	else: avg_score = 0 #TODO: decide if it should be based on average

	# avg_score = np.sum(word_scores)/len(word_scores)
	if return_hits:
		return (avg_score, hits)
